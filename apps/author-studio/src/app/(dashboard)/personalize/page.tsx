  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-white/[0.08]">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-zinc-400 font-mono">• DYNAMIC EXPERIENCE RESOLVER</span>
          </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/10 px-3 py-1 text-xs font-medium text-emerald-400 border border-emerald-500/20 font-mono">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="surface-card p-6 sm:p-8 rounded-3xl space-y-6 border border-white/[0.08] shadow-xl">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-zinc-400 font-mono">Dynamic Rule Classifier</span>
          </div>

          {/* New Segment Form */}
          <form onSubmit={handleCreateSegment} className="space-y-4 p-4 rounded-2xl bg-black/40 border border-white/[0.06]">
            ...
            <div>
              ...
              <input
                ...
                className="input-dark w-full px-3.5 py-2 text-xs text-white placeholder-zinc-500"
                required
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <label className="flex items-center gap-2 text-xs text-zinc-300 cursor-pointer pt-2">
                <input
                  ...
                  className="rounded border-white/20 bg-white/10 text-[#E8380D] focus:ring-[#E8380D] h-4 w-4"
                />
              ...
              <div>
                ...
                <input
                  ...
                  className="input-dark w-full px-3 py-1.5 text-xs text-white placeholder-zinc-500"
                />
              </div>
            </div>
            ...
            <button
              ...
              className="btn-firefly w-full rounded-xl px-4 py-2.5 text-xs font-semibold disabled:opacity-50 flex items-center justify-center gap-1.5 shadow-md shadow-[#E8380D]/20"
            >
          ...
          <div className="space-y-3">
            {segments.map((seg) => (
              <div
                key={seg.id}
                className="p-3.5 rounded-xl border border-white/[0.06] bg-white/[0.02] flex items-center justify-between hover:border-[#E8380D]/30 transition"
              >
                <div>
                  <h4 className="text-xs font-semibold text-white">{seg.name}</h4>
                  <div className="mt-1.5 flex flex-wrap gap-1.5">
                    {seg.ruleJson.isNew && (
                      <span className="rounded-md bg-blue-500/10 text-blue-400 px-2 py-0.5 text-[10px] font-mono border border-blue-500/20">
                        isNew: true
                      </span>
                    )}
                    {seg.ruleJson.referrerContains && (
                      <span className="rounded-md bg-amber-500/10 text-[#FFB347] px-2 py-0.5 text-[10px] font-mono border border-amber-500/20">
                        referrer: &quot;{seg.ruleJson.referrerContains}&quot;
                      </span>
                    )}
                    {!seg.ruleJson.isNew && !seg.ruleJson.referrerContains && (
                      <span className="rounded-md bg-zinc-500/10 text-zinc-400 px-2 py-0.5 text-[10px] font-mono">
                        global audience
                      </span>
                    )}
                  </div>
                </div>
                <span className="text-[10px] text-zinc-400 font-mono">ID: {seg.id.slice(0, 8)}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="surface-card p-6 sm:p-8 rounded-3xl space-y-6 border border-white/[0.08] shadow-xl">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-zinc-400 font-mono">Target Variant Matrix</span>
          </div>
          ...
          <form onSubmit={handleCreateVariant} className="space-y-4">
            ...
            <button
              ...
              className="btn-firefly w-full rounded-xl px-4 py-2.5 text-xs font-semibold disabled:opacity-50 flex items-center justify-center gap-2 shadow-md shadow-[#E8380D]/20"
            >
      ...
      <div className="surface-card p-6 sm:p-8 rounded-3xl space-y-6 border border-white/[0.08] shadow-xl">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px] font-mono text-[#FFB347] border border-[#E8380D]/30 bg-[#E8380D]/10 px-2.5 py-1 rounded-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              disabled={simulating}
              className="btn-secondary flex-1 inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2 text-xs font-bold hover:scale-[1.02] active:scale-[0.98] transition disabled:opacity-50"
            >
              <Play className="h-3.5 w-3.5 fill-current text-[#FFB347]" />
        ...
        {simResult && (
          <div className="rounded-2xl bg-black/60 p-4 border border-white/10 space-y-3 font-mono text-xs">
            <div className="flex items-center justify-between pb-3 border-b border-white/[0.08]">
              <span className="text-zinc-400">HTTP/2 200 OK • Edge Latency: 12ms</span>
              <span
            ...
            <pre className="overflow-x-auto text-[11px] leading-relaxed text-[#FFB347]">
